...
function Section(node, parent, retryHandler) {
...
Section.prototype = {
    ...
    toggleRetryButton: function(isEnabled) {
        var retryButton = this._bodyNode.querySelector('.image-box__retry');

        if (retryButton) {
            retryButton.disabled = !isEnabled;
        }
    },
    ...
    retry: function() {
        this._retryHandler(this._collectStateData());
    },
